...
function userFromDb(r){return{id:r.id,name:r.name||"",username:r.username,password:r.password,role:r.role||"lecteur"};}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {filtered.length===0
        ...
        : <div className="rounded-sm overflow-hidden" style={{background:C.cream}}>{filtered.map(m=><MemberCard key={m.id} member={m} annee={annee} onEdit={onEdit} onDelete={onDelete} onMarkPaid={onMarkPaid} onHistory={onHistory} canEdit={cedit} canDelete={cdel}/>)}</div>
      }
    </div>
  );
}
...
export default function App(){
  ...
  const handleLogin=u=>{setCurrent(u);sessionStorage.setItem("currentUserId",u.id);};
  ...
  const saveUser=async data=>{
    ...
    const{error}=await supabase.from("app_users").upsert({id:saved.id,name:saved.name,username:saved.username,password:saved.password,role:saved.role});
    if(error){toast2("Erreur: "+error.message);setSaving(false);return;}
    ...
    <main className="flex-1 min-w-0">
      <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-10">
        ...
        {tab==="members"&&<MembersTab members={members} annee={config.annee} search={mbrSearch} onSearch={setMbrSearch}
          ...
          onMarkPaid={markPaid} onHistory={m=>setHistModal(m)} currentUser={currentUser}/>}
