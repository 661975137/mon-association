...
/* ===================== ASSOC APP (app principale filtrée par association) ===================== */
function AssocApp({currentUser,association,onLogout}){
  return(
    <div style={{fontFamily:FB,background:C.paper,minHeight:"100vh"}}>
      <style>{FONT_IMPORT}</style>
      <div className="flex items-center justify-between px-6 py-3" style={{background:C.ink}}>
        <div>
          <div className="text-xs uppercase tracking-wide" style={{color:C.brass}}>{association.name}</div>
          <div className="text-sm font-medium" style={{color:C.cream}}>{currentUser.name||currentUser.username} · {currentUser.role}</div>
        </div>
        <button onClick={onLogout} className="flex items-center gap-2 text-xs px-3 py-2 rounded-sm" style={{border:"1px solid rgba(255,255,255,0.15)",color:"rgba(255,255,255,0.6)"}}>
          <LogOut size={13}/> Déconnexion
        </button>
      ...
      <div className="p-6 max-w-2xl mx-auto text-center py-20">
        <CheckCircle2 size={40} className="mx-auto mb-4" style={{color:C.sage}}/>
        <h2 style={{fontFamily:FD,fontStyle:"italic",fontSize:"24px",color:C.ink}} className="mb-2">Espace {association.name}</h2>
        <p style={{color:C.slate}}>L'application complète est chargée ici avec toutes les données filtrées pour votre association.</p>
        <p className="text-xs mt-3" style={{color:C.slate,fontFamily:FM}}>association_id: {association.id}</p>
      </div>
    </div>
  );
}
