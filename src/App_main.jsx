...
/* === SUPABASE MAPPERS === */
function memberToDb(m,assocId=null){return{id:m.id,name:m.name,email:m.email||null,phone:m.phone||null,cotisation_amount:Number(m.cotisationAmount)||0,join_date:m.joinDate||null,cotisations:m.cotisations||{},commune:m.commune||null,...(assocId?{association_id:assocId}:{})};}
...
export default function App({externalUser=null,externalAsso=null,externalLogout=null}){
  ...
  const assocId=externalAsso?.id||null;
    ...
    (async()=>{
      try{
        const [{data:ud},{data:md},{data:td},{data:cd},{data:rd},{data:ed},{data:sd}]=await Promise.all([
          assocId?supabase.from("app_users").select("*").eq("association_id",assocId):supabase.from("app_users").select("*"),
          assocId?supabase.from("members").select("*").eq("association_id",assocId):supabase.from("members").select("*"),
          assocId?supabase.from("transactions").select("*").eq("association_id",assocId).order("date",{ascending:false}):supabase.from("transactions").select("*").order("date",{ascending:false}),
          supabase.from("config").select("*").eq("id",assocId||1).maybeSingle(),
          assocId?supabase.from("reunions").select("*").eq("association_id",assocId).order("date",{ascending:false}):supabase.from("reunions").select("*").order("date",{ascending:false}),
          assocId?supabase.from("evenements").select("*").eq("association_id",assocId).order("date",{ascending:false}):supabase.from("evenements").select("*").order("date",{ascending:false}),
          assocId?supabase.from("sanctions").select("*").eq("association_id",assocId).order("date",{ascending:false}):supabase.from("sanctions").select("*").order("date",{ascending:false})
        ]);
  ...
  const markPaid=async m=>{
    ...
    const [{error:me},{error:te}]=await Promise.all([
      supabase.from("members").upsert(memberToDb(upd,assocId)),
      supabase.from("transactions").insert(txToDb(newTx,assocId))
    ]);
  ...
  const markSanctionPaid=async s=>{
    ...
    if(s.montant>0){
      ...
      await supabase.from("transactions").insert(txToDb(newTx,assocId));
      setTxs(v=>[newTx,...v]);
    }
  ...
  if(!externalUser&&users.length===0) return<SetupScreen onSetup={handleSetup}/>;
  if(!externalUser&&!currentUser) return<LoginScreen users={users} onLogin={handleLogin}/>;
